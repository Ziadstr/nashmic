# Summary

[Introduction](introduction.md)

# Getting Started

- [Getting Started](getting-started.md)

# Language Basics

- [Variables & Constants](basics/variables.md)
- [Functions](basics/functions.md)
- [Control Flow](basics/control-flow.md)
- [Strings & Interpolation](basics/strings.md)

# Type System

- [Structs](types/structs.md)
- [Enums (Tagged Unions)](types/enums.md)
- [Optionals](types/optionals.md)

# Error Handling

- [Result Type (natije)](error-handling/natije.md)
- [Defer (ba3dain)](error-handling/defer.md)

# Advanced Features

- [Pattern Matching](advanced/pattern-matching.md)
- [Methods & Impl Blocks](advanced/methods.md)

# Reference

- [Keywords](reference/keywords.md)
- [Types](reference/types.md)
- [Built-in Functions](reference/builtins.md)
- [Compiler Messages](reference/compiler-messages.md)

# Examples

- [Examples](examples.md)
